...
export const verifyMembershipPayment = cache(async (stackAuthId: string) => {
    ...
    const [member] = await db
        ...
        .from(memberTable)
        .where(eq(memberTable.stackAuthId, stackAuthId));
    ...
    const orderId = await redisClient.hGet(`payment:membership:${stackAuthId}`, 'orderId');
    if (!orderId) {
    ...
    try {
        ...
        if (paymentId) {
            ...
            const expiryDate = await updateMemberExpiryDate(stackAuthId, 'stackAuthId');

            // Delete key from Redis since it is no longer needed
            await redisClient.del(`payment:membership:${stackAuthId}`);
