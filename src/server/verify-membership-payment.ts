...
export const verifyMembershipPayment = cache(async (id: string) => {
    ...
    const orderId = await redisClient.hGet(`payment:membership:${id}`, 'orderId');
    if (!orderId) {
    ...
    try {
        ...
        if (paymentId) {
            ...
            const expiryDate = await updateMemberExpiryDate(id, 'id');

            // Delete key from Redis since it is no longer needed
            await redisClient.del(`payment:membership:${id}`);
