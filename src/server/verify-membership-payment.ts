...
export const verifyMembershipPayment = cache(async (logtoID: string) => {
    ...
    const [member] = await db
        ...
        .from(memberTable)
        .where(eq(memberTable.logtoID, logtoID));
    ...
    const orderId = await redisClient.hGet(`payment:membership:${logtoID}`, 'orderId');
    if (!orderId) {
    ...
    try {
        ...
        if (paymentId) {
            ...
            const expiryDate = await updateMemberExpiryDate(logtoID, 'logtoID');

            // Delete key from Redis since it is no longer needed
            await redisClient.del(`payment:membership:${logtoID}`);
